...
public class AffindaAPI {
    ...
    public static List<String> extractSkills(String resultJson) {
        ...
        try {
            ...
            System.out.println("RAW JSON: " + resultJson);
            JSONObject data = result.has("data") ? result.getJSONObject("data") : new JSONObject();
            System.out.println("FULL DATA: " + data.toString());
            JSONArray skillArray = data.optJSONArray("skills");
            System.out.println("SKILLS RAW: " + skillArray);
            if (skillArray != null) {
 ...
 // ✅ Extract work experience from Affinda JSON result
    ...
    public static int calculateMatchScoreFromText(String resumeText, String jobSkillsCsv) {
        if (resumeText == null || resumeText.isEmpty() || jobSkillsCsv == null || jobSkillsCsv.isEmpty()) {
            return 0;
        }
        ...
        if (total == 0) return 0;
        return (int) ((matched * 100.0) / total);
    }
}
